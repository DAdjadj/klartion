...
from .. import config, db, enablebanking, licence, sync, notion
...
def setup():
    ...
    if request.method == "POST":
        # Validate licence key first
        key = request.form.get("licence_key", "").strip()
        result = licence.validate(key)
        if not result["valid"]:
            error = f"Licence key invalid: {result['error']}"
        else:
            # In self-hosted mode config comes from .env, so we just verify
            # all required fields are present via environment
...
def connect():
    ...
    if request.method == "POST":
        ...
        if not bank_name or not bank_country:
            ...
            try:
                auth_url = enablebanking.get_auth_url(bank_name, bank_country)
                return redirect(auth_url)
            except Exception as e:
                logger.error("Failed to get auth URL: %s", e)
...
def callback():
    ...
    try:
        ok = enablebanking.complete_auth(code=request.args.get("code", ""))
        if ok:
            return redirect(url_for("connect") + "?success=1")
        else:
            return redirect(url_for("connect") + "?error=auth_failed")
    except Exception as e:
        logger.error("OAuth callback exception: %s", e)
...
def sync_now():
    """Manual sync trigger from the status page."""
    import threading
