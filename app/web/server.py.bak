...
def setup_notifications():
    ...
    if request.method == "POST":
        ...
        if not klartion_url or not email or not smtp_user or not smtp_pass:
            ...
            # Start scheduler now that we have full config
...
def status():
    ...
    # Check if last sync failed due to licence issue AND no active instance
