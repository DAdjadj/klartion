...
def save_simplefin_token(bank_name, bank_country, provider_account_id, provider_credentials, user_id="default", start_sync_date=""):
    ...
    conn = get_conn()
    cur = conn.execute("""
        INSERT INTO tokens (user_id, bank_name, bank_country, provider, provider_credentials, provider_account_id, sync_mode, license_seat_id, start_sync_date)
        VALUES (?, ?, ?, 'simplefin', ?, ?, 'transactions', ?, ?)
    """, (user_id, bank_name, bank_country, provider_credentials, provider_account_id, str(uuid.uuid4()), start_sync_date))
    conn.commit()
    conn.close()
    return cur.lastrowid
...
def update_token_fields(token_id, **fields):
    allowed = {"access_token", "session_id", "bank_name", "bank_country", "expires_at", "start_sync_date", "last_sync_at", "last_balance", "last_balance_currency", "provider_credentials", "license_seat_id", "skip_pending", "provider_account_id"}
